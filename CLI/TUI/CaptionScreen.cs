...
namespace image_flip_bosch.CLI.TUI
{
  ...
  internal sealed class CaptionScreen : NanoScreen
  {
    ...
    public CaptionScreen(ConsoleWindowSystem ws, Meme meme, string? imagePath, bool customPositions, MemeCreationBox[]? previous = null)
      : base(ws, "Caption")
    {
      _meme = meme;
      ...
      for (int i = 0; i < count; i++)
      {
        ...
        if (previous is not null && i < previous.Length)
        {
          ...
          if (p.X is int x) box.X = x;
          if (p.Y is int y) box.Y = y;
          if (p.Width is int w) box.Width = w;
          if (p.Height is int h) box.Height = h;
        }
        _boxes.Add(box);
      }
      ...
      GridControl left = Controls.Grid()
        ...
        .Rows(column.Select(_ => GridLength.Auto()).ToArray())
    ...
    private string BoxLabel(int i)
    {
      ...
      if (!CustomPositions)
        return $" [{b.ColorHex}]{marker} ■[/] {Chrome.MutedText($"{Label(i, _boxes.Count)}  template position")}";
      return $" [{b.ColorHex}]{marker} ■[/] {Chrome.MutedText($"{Label(i, _boxes.Count)}  x{b.X} y{b.Y}  {b.Width}x{b.Height}")}";
    }
